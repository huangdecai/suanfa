...
class GameHelper:
    ...
    def GetCardsState(self, image,handCount):
        ...
        cardStartPos = pyautogui.locate(needleImage=self.Pics["card_edge"], haystackImage=image,
                                        region=(1, 460, 880, 110), confidence=0.80)
        ...
        for i in range(0, MAX_CARD_COUNT):
            ...
            result2 = LocateOnImage(imgCv, self.PicsCV["card_overlap2"], region=(temp_x, spaceY - 10, spaceX, 40),
                                    confidence=0.85)
            result3 = LocateOnImage(imgCv, self.PicsCV["card_overlap3"], region=(temp_x, spaceY - 10, spaceX, 40),
                                    confidence=0.85)
            if (result is not None) and ((result2 is None and result2 is result3)):
                checkSelect = 1
            ...
            while ci < len(AllCardsNC):
                if "r" in AllCardsNC[ci] or "b" in AllCardsNC[ci]:
                    ...
                    result = LocateOnImage(imgCv, self.PicsCV["o" + AllCardsNC[ci]],
                                           region=(sx + spaceX * i, spaceY - checkSelect * 25, sw, 68), confidence=0.89)
