...
class GameHelper:
    ...
    def Screenshot(self, region=None):  # -> (im, (left, top))
        ...
        hwnd = self.Handle
        if self.bTest == True:
            im = Image.open(r"testRunFast3.png")
    ...
    def GetCardsState(self, image,handCount):
        ...
        cardStartPos = pyautogui.locate(needleImage=self.Pics["card_edge"], haystackImage=image,
                                        region=(10, 430, 880, 110), confidence=0.80)
        ...
        for i in range(0, handCount):
            ...
            checkSelect = 0
            result = LocateOnImage(imgCv, self.PicsCV["card_overlap"], region=(temp_x, spaceY - 14, spaceX, 14),
                                   confidence=0.80)
            result2 = LocateOnImage(imgCv, self.PicsCV["card_overlap2"], region=(temp_x, spaceY - 15, spaceX, 40),
                                    confidence=0.85)

            result3 = LocateOnImage(imgCv, self.PicsCV["card_overlap3"], region=(temp_x, spaceY - 14, spaceX, 14),
                                    confidence=0.85)
            if ((result is not None) or (result3 is not None)) and (result2 is None):
                checkSelect = 1
