...
class GameHelper:
    ...
    def Screenshot(self, region=None):  # -> (im, (left, top))
        self.Handle = win32gui.FindWindow("LDPlayerMainFrame", "雷电模拟器")
        hwnd = self.Handle
        #im = Image.open(r"testRunFast.png")
    ...
    def GetCardsState(self, image,handCount):
        ...
        cardStartPos = pyautogui.locate(needleImage=self.Pics["card_edge"], haystackImage=image,
                                        region=(155, 500, 800, 60), confidence=0.80)
        ...
        for i in range(0, MAX_CARD_COUNT):
            ...
            result2 = LocateOnImage(imgCv, self.PicsCV["card_overlap2"], region=(temp_x, spaceY - 25, spaceX, 40),
                                    confidence=0.85)
            result3 = LocateOnImage(imgCv, self.PicsCV["card_overlap3"], region=(temp_x, spaceY - 25, spaceX, 40),
                                    confidence=0.85)
            if (result is not None) and (result2 is None) and (result3 is None):
                checkSelect = 1
