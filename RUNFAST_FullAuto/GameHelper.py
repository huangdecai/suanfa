...
class GameHelper:
    ...
    def GetCardsState(self, image,handCount):
        ...
        cardStartPos = pyautogui.locate(needleImage=self.Pics["card_edge"], haystackImage=image,
                                        region=(2, 368, 880, 110), confidence=0.85)
        ...
        for i in range(0, MAX_CARD_COUNT):
            ...
            checkSelect = 0
            result = LocateOnImage(imgCv, self.PicsCV["card_overlap"], region=(temp_x, 445, sw, 80), confidence=0.83)
            if result is not None:
                checkSelect = 1
    ...
    def \
        ...
        tryCount = 10
        cardStartPos = pyautogui.locate(needleImage=self.Pics["card_edge"], haystackImage=image,
                                        region=(2, 368, 880, 110), confidence=0.80)
        ...
        for i in range(0, MAX_CARD_COUNT):
            ...
            while ci < len(AllCardsNC):
                if "r" in AllCardsNC[ci] or "b" in AllCardsNC[ci]:
                    outerBreak = False
                    result = LocateOnImage(imgCv, self.PicsCV["m" + AllCardsNC[ci]], region=(sx + spaceX * i, 400 - checkSelect * 25, sw, 68), confidence=0.89)

                    if result is not None:
                        cardPos = (sx + spaceX * i + sw // 2, 400 - checkSelect * 25 + sh // 2)
                        cardSearchFrom = ci
                        currCard = AllCardsNC[ci][1]
                        cardInfo = (currCard, cardPos)
                    ...
                    outerBreak = False
                    for card_type in ["r", "b"]:
                        result = LocateOnImage(imgCv, self.PicsCV["m" + card_type + AllCardsNC[ci]], region=(sx + spaceX * i, 400 - checkSelect * 25, sw, 68), confidence=0.91)
