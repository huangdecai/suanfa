...
def _load_model(position, model_path, model_type):
    ...
    if model_type == "general":
        ...
        # model = model_dict_resnet[position]()py
    else:
...
class DeepAgent:
    ...
    def act(self, infoset):
        ...
        tmpConfidence=-99999999.0
        if len(best_action)==0:
           for i in range(0,len(infoset.legal_actions)):
                if len(infoset.legal_actions[i])>0 and y_pred[i]>tmpConfidence:
                    best_action=infoset.legal_actions[i]
                    tmpConfidence=y_pred[i]
                    best_action_confidence=y_pred[i]
