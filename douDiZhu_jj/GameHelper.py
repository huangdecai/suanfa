    ...
    def GetCards(self, image,handCount):
        ...
        for i in range(0, MAX_CARD_COUNT):
            ...
            while ci < len(AllCardsNC):
                if "r" in AllCardsNC[ci] or "b" in AllCardsNC[ci]:
                    outerBreak = False
                    tmpcardstr="m" + AllCardsNC[ci]
                    result = LocateOnImage(imgCv, self.PicsCV["m" + AllCardsNC[ci]], region=(sx + spaceX * i, spaceY- checkSelect * 25, sw, 90), confidence=0.95)
                    ...
                    outerBreak = False
                    for card_type in ["r", "b"]:
                        tmpstr="m" + card_type + AllCardsNC[ci]
                        result = LocateOnImage(imgCv, self.PicsCV[tmpstr], region=(sx + spaceX * i, spaceY - checkSelect * 25, sw, 100), confidence=0.88)
                        if result is not None:
                            cardPos = (sx + spaceX * i + sw // 2, spaceY - checkSelect * 25 + sh // 2)
                            cardSearchFrom = ci
                            currCard = AllCardsNC[ci]
                ...
                if "r" in AllCardsNC[ci] or "b" in AllCardsNC[ci]:
                    ...
                    for card_type in ["r", "b"]:
                        result = LocateOnImage(imgCv, self.PicsCV["o" + card_type + AllCardsNC[ci]],
                                               region=(sx + spaceX * i, spaceY - checkSelect * 25, sw, 98),
                                               confidence=0.86)
                        if result is not None:
                            cardPos = (sx + spaceX * i + sw // 2, spaceY - checkSelect * 25 + sh // 2)
                            cardSearchFrom = ci
