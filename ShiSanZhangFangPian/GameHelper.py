...
class GameHelper:
    ...
    def LocateOnScreenEX(self, templateName, region, confidence=0.9):
        newregion=(region[0],region[1],region[2]-40,region[3])
        image, _ = self.Screenshot()
        return pyautogui.locate(needleImage=self.Pics[templateName],
                                haystackImage=image, region=region, confidence=confidence)
    def ClickOnImage(self, templateName, region=None, confidence=0.9):
        ...
        if result is not None:
            self.LeftClick((result[0]+30, result[1]+15))
    def ClickOnImageEX(self, templateName, region=None, confidence=0.9):
        image, _ = self.Screenshot()
        result = pyautogui.locate(needleImage=self.Pics[templateName], haystackImage=image, confidence=confidence, region=region)
        if result is not None:
            self.LeftClickEX((result[0]+30, result[1]+15))
    def GetCardsState(self, image,handCount):
        ...
        cardStartPos = pyautogui.locate(needleImage=self.Pics["card_edge"], haystackImage=image,
                                        region=(10, 760, 880, 110), confidence=0.80)
        ...
        for i in range(0, handCount):
            temp_x=sx+9 + spaceX *(i)
            if temp_x>=(SCREEN_WIDTH-50) :
                break
            print("GetCardsState-LocateOnImage:", temp_x, sw)
            checkSelect = 0
            result = LocateOnImage(imgCv, self.PicsCV["card_overlap"], region=(temp_x, spaceY - 45, spaceX, 30),
                                   confidence=0.85)
            result2 = LocateOnImage(imgCv, self.PicsCV["card_overlap2"], region=(temp_x, spaceY - 25, spaceX, 40),
                                    confidence=0.85)
    ...
    def \
        ...
        for i in range(0, MAX_CARD_COUNT):
            ...
            if temp_x >= (SCREEN_WIDTH - 50):
                break
            checkSelect = 0
            result = LocateOnImage(imgCv, self.PicsCV["card_overlap"], region=(temp_x, spaceY-45, spaceX, 30), confidence=0.85)
            result2 = LocateOnImage(imgCv, self.PicsCV["card_overlap2"], region=(temp_x, spaceY-25, spaceX, 40), confidence=0.85)
            if (result is not None) and (result2 is  None):
                checkSelect = 1
            ...
            while ci < len(AllCardsNC):
                outerBreak = False
                if "r" in AllCardsNC[ci] or "b" in AllCardsNC[ci]:
                    result = LocateOnImage(imgCv, self.PicsCV["m" + AllCardsNC[ci]], region=(sx + spaceX * i, spaceY- checkSelect * 31, sw, 68), confidence=0.87)

                    if result is not None:
                        cardPos = (sx + spaceX * i + sw // 2, spaceY - checkSelect * 31 + sh // 2)
                        cardSearchFrom = ci
                        currCard = AllCardsNC[ci][1]
                        cardInfo = (currCard, cardPos)
                    ...
                    for card_type in ["r", "b"]:
                        result = LocateOnImage(imgCv, self.PicsCV["m" + card_type + AllCardsNC[ci]], region=(sx + spaceX * i, spaceY - checkSelect * 31, sw, 68), confidence=0.87)
                        if result is not None:
                            cardPos = (sx + spaceX * i + sw // 2, spaceY - checkSelect * 31 + sh // 2)
                            cardSearchFrom = ci
                            currCard = AllCardsNC[ci]
                            ...
                            for card_color in ["0","1","2","3"]:
                                result = LocateOnImage(imgCv, self.PicsCV["color" + card_color],
                                                       region=(sx+1 + spaceX * i, spaceY - checkSelect * 31+23, sw, 33),
                                                       confidence=0.95)
                                if result :
