...
class GameHelper:
    ...
    def GetCardsState(self, image,handCount):
        ...
        cardStartPos = pyautogui.locate(needleImage=self.Pics["card_edge"], haystackImage=image,
                                        region=(10, 413, 880, 110), confidence=0.85)
    ...
    def \
        ...
        for i in range(0, MAX_CARD_COUNT):
            ...
            while ci < len(AllCardsNC):
                outerBreak = False
                if "r" in AllCardsNC[ci] or "b" in AllCardsNC[ci]:
                    result = LocateOnImage(imgCv, self.PicsCV["m" + AllCardsNC[ci]], region=(sx + spaceX * i, spaceY- checkSelect * 25, sw, 68), confidence=0.93)
                    ...
                    for card_type in ["r", "b"]:
                        result = LocateOnImage(imgCv, self.PicsCV["m" + card_type + AllCardsNC[ci]], region=(sx + spaceX * i, spaceY - checkSelect * 25, sw, 80), confidence=0.87)
                        if result is not None:
                            cardPos = (sx + spaceX * i + sw // 2, spaceY - checkSelect * 25 + sh // 2)
                            cardSearchFrom = ci
                            currCard = AllCardsNC[ci]
                            ...
                            for card_color in ["0","1","2","3"]:
                                result = LocateOnImage(imgCv, self.PicsCV["color" + card_color],
                                                       region=(sx + spaceX * i, spaceY - checkSelect * 25+30, sw, 30),
                                                       confidence=0.95)
                                if result :
