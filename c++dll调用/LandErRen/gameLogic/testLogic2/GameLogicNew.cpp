		...
		for (BYTE i = 0; i < cbSameCardResultCount; i++)
		{
			...
			for (BYTE j = cbTakeCardCount - 1; j < CountArray(AnalyseResult.cbBlockCount); j++)
			{
				for (BYTE k = 0; k < AnalyseResult.cbBlockCount[j]; k++)
				{
					...
					{
						if (cbTakeCardCount == 1 && (cbSameCount == 3 || cbSameCount == 4))
						{
							//恢复扑克，防止分析时改变扑克
							BYTE  cbTmpCardData[FULL_COUNT] = { 0 };
							CopyMemory(cbTmpCardData, cbHandCardData, cbHandCardCount);
							...
							{
								for (int m = 0; m < cbRemainCardCount; m++)
								{
									//复制带牌
...
int CGameLogicNew::SearchMutilType(const BYTE cbHandCardData[], BYTE cbHandCardCount, vector<tagOutCardResultNew>& vecMinTypeCardResult, tagOutCardResultNew & OutCardResult)
{
	...
	for (int i = 0; i < vecMinTypeCardResult.size(); i++)
	{
		...
		if ((type>CT_SINGLE && type < CT_FOUR_TAKE_ONE) || bSpecal)
		{
...
*********************************************************************/
//组合算法
VOID CGameLogicNew::Combination(BYTE cbCombineCardData[], int cbResComLen,  BYTE cbResultCardData[MAX_RESULT_COUNT][8], int &cbResCardLen,BYTE cbSrcCardData[] , BYTE cbCombineLen1, BYTE cbSrcLen, const BYTE cbCombineLen2)
{
...
bool CGameLogicNew::OutCardShengYuFenCheck(BYTE cbHandCardCount, const BYTE * cbHandCardData, const BYTE cbTurnCardData[], BYTE cbTurnCardCount, WORD wOutCardUser, float MinTypeScoreVec, vector<tagOutCardResultNew> &vecMinTypeCardResult, tagOutCardResultNew & OutCardResult)
{
	...
	BYTE MaxResultLogic = 0;
	for (int i = 0; i < SearchCardResult.cbSearchCount; i++)
	{

		if (SearchCardResult.cbCardCount[i]==1)
		{
			singleSearchData.push_back(SearchCardResult.cbResultCard[i][0]);
		}
		...
		if (cbReserveCardCount > 0)
		{
			...
			else if (NoOutCardUserHandCardCountCheck(m_wMeChairID, 1))
			{
				...
				{
					BYTE tmpLogic = GetCardLogicValue(SearchCardResult.cbResultCard[i][0]);
					if (tmpLogic> MaxResultLogic)
					{
						MaxResultLogic = tmpLogic;
						MinTypeScore = tempMinTypeScore;
						minTypeCount = tempMinTypeCount;
						resultIndex = i;
						vecMinTypeCardResultBak = TempMinTypeCardResult[i];
	...
	if ((resultIndex != -1) && bExistBiYing==false)
	{
		...
		if (MinTypeScoreVec - tableScore[resultIndex] >= tempCardScore && NoOutCardUserHandCardCountCheck(wOutCardUser,2,false,false))
		{
		
			if (tempCardType == CT_MISSILE_CARD)
			{
				if (CheckOutOneTypeWillWin(cbHandCardData, cbHandCardCount, SearchCardResult.cbResultCard[resultIndex], SearchCardResult.cbCardCount[resultIndex], OutCardResult))
				{
				}
				else{
					bool bZhiJieChu = false;
					BYTE tmpCardData[MAX_COUNT] = { 0 };
					BYTE tmpCardCount = cbHandCardCount;
					CopyMemory(tmpCardData, cbHandCardData, cbHandCardCount);
					if (RemoveCard(SearchCardResult.cbResultCard[resultIndex], SearchCardResult.cbCardCount[resultIndex], tmpCardData, tmpCardCount))
					{
						tmpCardCount = tmpCardCount - SearchCardResult.cbCardCount[resultIndex];
					}
					vector<tagOutCardResultNew>   tmpMinTypeCardResult;
					for (int i = 1; i < vecMinTypeCardResult.size(); i++)
					{
						tmpMinTypeCardResult.push_back(vecMinTypeCardResult[i]);
					}
					tagOutCardResultNew tmpOutCardResult = {};
					int outIndex = YouXianDaNengShouHuiCard(tmpCardData, tmpCardCount, tmpMinTypeCardResult, tmpOutCardResult, bZhiJieChu);
					if (bZhiJieChu == false)
					{
						int tempTypeCount = 0;
						for (int i = 0; i < tmpMinTypeCardResult.size(); i++)
						{
							if (NoOutCardUserHandCardCountCheck(m_wMeChairID, tmpMinTypeCardResult[i].cbCardCount,false,false))
							{
								tempTypeCount++;
							}
						}
			...
			else
			{
				...
				if (DoubleChaiFenCheck(TempMinTypeCardResult[resultIndex], cbHandCardData, cbHandCardCount,wOutCardUser))
				{
					if (NoOutCardUserHandCardCountCheck(wOutCardUser,1,true,false))
					{
						resultIndex = -1;
					}
				}
			...
			{
				if (tempCardType == CT_SINGLE || tempCardType == CT_DOUBLE)
				{
					bool bIntersect = false;
					for (int i = 0; i < vecMinTypeCardResult.size();i++)
					{
						if (vecMinTypeCardResult[i].cbCardType == CT_DOUBLE_LINE || vecMinTypeCardResult[i].cbCardType == CT_THREE_LINE)
						{
							if (vecMinTypeCardResult[i].cbCardType == CT_DOUBLE_LINE&&cbHandCardData[0] == vecMinTypeCardResult[i].cbResultCard[0])
							{
								//CT_DOUBLE_LINE包含了手上最大的单牌了,应该拆了
								break;
							}
							if (CheckTwoArrayIntersect(vecMinTypeCardResult[i].cbResultCard, vecMinTypeCardResult[i].cbCardCount, SearchCardResult.cbResultCard[resultIndex], SearchCardResult.cbCardCount[resultIndex]))
							{
								bIntersect = true;
								for (int j = 0; j < vecMinTypeCardResult.size(); j++)
								{
									if (vecMinTypeCardResult[j].cbCardType == CT_SINGLE)
									{
										if (GetCardLogicValue(vecMinTypeCardResult[j].cbResultCard[0]) == GetCardLogicValue(SearchCardResult.cbResultCard[resultIndex][0]))
									   {
										   bIntersect = false;
										   break;
									   }
					...
					int tmpCount = vecMinTypeCardResultBak.size() - vecMinTypeCardResult.size();
					if (bIntersect && (NoOutCardUserHandCardCountCheck(wOutCardUser,2,true,false)))
					{
						resultIndex = -1;
					}
					else if ((tmpCount >= 3) && (NoOutCardUserHandCardCountCheck(wOutCardUser, 2,true,false)))
					{
						resultIndex = -1;
					}
					else if (tempCardType == CT_SINGLE)
					{
						int count = 0;
						int index = 0;
						...
						if (count == 1 && (GetCardLogicValue(SearchCardResult.cbResultCard[resultIndex][0]) != 15))
						{
							if (GetCardLogicValue(vecMinTypeCardResultBak[index].cbResultCard[0]) > GetCardLogicValue(SearchCardResult.cbResultCard[resultIndex][0]))
							{
								OutCardResult.cbCardCount = 1;
...
bool CGameLogicNew::SearchOneOrTwoFromThreeTake(vector<tagOutCardResultNew> &vecMinTypeCardResult, BYTE cbOutCardType, tagOutCardResultNew &OutCardResult)
{
	...
	for (int j = 0; j < vecMinTypeCardResult.size(); j++)
	{
		...
		if (cbOutCardType == CT_DOUBLE)
		{
			if (vecMinTypeCardResult[j].cbCardType == CT_THREE_TAKE_TWO)
			{
				...
				{
					vecResultData.push_back(vecMinTypeCardResult[j].cbResultCard[vecMinTypeCardResult[j].cbCardCount - 1 - k]);
					vecResultData.push_back(vecMinTypeCardResult[j].cbResultCard[vecMinTypeCardResult[j].cbCardCount - 2 - k]);
					break;
				}
			}
		}
	...
	if (vecResultData.size()>0)
	{
		...
		{
			if (GetCardLogicValue(first) < GetCardLogicValue(first))
			{
				return true;
			}
			return false;
...
VOID CGameLogicNew::BankerOutCard(const BYTE cbHandCardData[], BYTE cbHandCardCount, tagOutCardResultNew & OutCardResult)
{
	...
	if (FindMaxTypeTakeOneType(cbHandCardData, cbHandCardCount, 0, 0, vecMinTypeCardResult, OutCardResult))
	{
...
VOID CGameLogicNew::BankerOutCard(const BYTE cbHandCardData[], BYTE cbHandCardCount, WORD wOutCardUser, const BYTE cbTurnCardData[], BYTE cbTurnCardCount, tagOutCardResultNew & OutCardResult)
{
	...
	if (FindMaxTypeTakeOneType(cbHandCardData, cbHandCardCount, cbOutCardType, cbTurnCardCount, vecMinTypeCardResult, OutCardResult))
	{
		return;
	}
...
bool CGameLogicNew::FindMaxTypeTakeOneType(const BYTE cbHandCardData[], BYTE cbHandCardCount, BYTE cbTurnCardType, BYTE cbTurnCardCount, vector<tagOutCardResultNew> & vecMinTypeCardResult, tagOutCardResultNew &OutCardResult)
{
	...
	for (int i = 0; i < vecMinTypeCardResult.size();i++)
	{
		if (vecMinTypeCardResult[i].cbCardType == CT_BOMB_CARD || vecMinTypeCardResult[i].cbCardType == CT_MISSILE_CARD || cbTurnCardType == 0)
		{
			...
			vecMinTypeCardResult[i].cbCardCount == cbTurnCardCount)
		{
			BYTE tmpTurnCard[MAX_COUNT] = { 0 };
			CopyMemory(tmpTurnCard, vecMinTypeCardResult[i].cbResultCard, vecMinTypeCardResult[i].cbCardCount);
			bool bExistMax = SearchOtherHandCardThan(tmpTurnCard, vecMinTypeCardResult[i].cbCardCount, false);
...
bool CGameLogicNew::SearchOutCardErRen(BYTE cbHandCardData[], BYTE cbHandCardCount, BYTE cbTurnCardData[], BYTE cbTurnCardCount, BYTE	DiscardCard[], BYTE cbDiscardCardCount, BYTE cbCardDataEx[], BYTE MaxCard[], tagOutCardResultNew & OutCardResult)
{
	...
	m_cbUserCardCount[1] =  (37 - cbHandCardCount - cbDiscardCardCount);
	if (m_cbCardDataEx[0]>0)
	{
		m_cbUserCardCount[1] -= m_cbCardDataEx[0];
	}
	m_cbOthreRangCardCount = m_cbCardDataEx[1];
	//玩家判断
	WORD wUndersideOfBanker = (m_wBankerUser + 1) % GAME_PLAYER;	//地主下家
	WORD wUpsideOfBanker = (wUndersideOfBanker + 1) % GAME_PLAYER;	//地主上家
...
VOID CGameLogicNew::ErZhuDongOutCard(const BYTE cbHandCardData[], BYTE cbHandCardCount, tagOutCardResultNew & OutCardResult)
{
	...
	BaoDanJiaoYan(OutCardResult, 1, cbHandCardData);
	return;
}
...
VOID CGameLogicNew::ErBeiDongOutCard(const BYTE cbHandCardData[], BYTE cbHandCardCount, WORD wOutCardUser, const BYTE cbTurnCardData[], BYTE cbTurnCardCount, tagOutCardResultNew & OutCardResult)
{
	...
	//当两个人中有其中一人手牌小于等于2了,那么地主必须拆牌来压了
	if (OutCardResult.cbCardCount == 1 && (m_cbUserCardCount[wUndersideUser] == 1) && cbTurnCardCount == 1)
	{
