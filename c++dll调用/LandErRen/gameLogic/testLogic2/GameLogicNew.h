...
public:
	//组合算法
	VOID Combination(BYTE cbCombineCardData[], int cbResComLen, BYTE cbResultCardData[MAX_RESULT_COUNT][8], int &cbResCardLen, BYTE cbSrcCardData[], BYTE cbCombineLen1, BYTE cbSrcLen, const BYTE cbCombineLen2);
	//分析炸弹
...
public:
	...
	//从三带一或者三带二中找出最小的对
	bool SearchOneOrTwoFromThreeTake(vector<tagOutCardResultNew> &vecMinTypeCardResult, BYTE cbOutCardType, tagOutCardResultNew &OutCardResult);
	//从三带一或者三带二中交换最小的带牌
	...
	//判断是否是最大类型带上拖一类型
	bool FindMaxTypeTakeOneType(const BYTE cbHandCardData[], BYTE cbHandCardCount, BYTE cbTurnCardType, BYTE cbTurnCardCount, vector<tagOutCardResultNew> & vecMinTypeCardResult, tagOutCardResultNew &OutCardResult);
	//判断是否牌面最大
...
public:
	...
	//出牌搜索
	bool SearchOutCardErRen(BYTE cbHandCardData[], BYTE cbHandCardCount, BYTE cbTurnCardData[], BYTE cbTurnCardCount, BYTE	DiscardCard[], BYTE cbDiscardCardCount, BYTE cbCardDataEx[], BYTE MaxCard[], tagOutCardResultNew & OutCardResult);
	//地主出牌（先出牌）
