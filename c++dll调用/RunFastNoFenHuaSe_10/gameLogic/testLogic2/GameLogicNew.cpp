...
const BYTE	CGameLogicNew::m_cbCardData[FULL_COUNT]=
{
	...
	0x01, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0A, 0x0B, 0x0C, 0x0D,	//梅花 A - K
	0x01, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0A, 0x0B, 0x0C, 0x0D,	//红桃 A - K
	0x01, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0A, 0x0B, 0x0C, 0x0D,	//黑桃 A - K
	//0x4E,0x4F,
};
...
int multithreadSort(vector<int> &vecHandCardCount, int sum, vector<tagOutCardResultNew>& resultVec, vector<vector<vector<tagOutCardResultNew> >> &resultAllVec, BYTE const cbHandCardData[], BYTE const cbHandCardCount){
	...
	for (int i = 0; i < num_threads; i++){//这里只分配子线程的任务序列,主线程也会执行一个排序任务
		...
		threads[i] = thread(accumulate_block(), std::ref(vecHandCardCount), start, end, 0, sum, std::ref(resultVec), std::ref(resultAllVec[i]),cbHandCardData, cbHandCardCount);//每个子线程的子序列分配  
	}
