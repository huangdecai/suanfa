...
VOID CGameLogicNew::SearchOutCardShiSanZhang(const BYTE cbHandCardData[], BYTE cbHandCardCount, tagOutCardResultNew & OutCardResult)
{
	...
	int minTypeCount = FindCardKindMinNum(cbHandCardData, cbHandCardCount, CardTypeResult, vecMinTypeCardResult, vecMinTypeCardResultShao, tempMinTypeScore, tempMinTypeScoreShao, biYing);
	float scoreEX = 0.0;
	bool bChongSan = bIsChongSanModel(vecMinTypeCardResult, scoreEX);
	...
	for (int i = 0; i < vecMinTypeCardResultShao.size(); i++)
	{
		if (vecMinTypeCardResultShao[i].size() > 0 && ((tempMinTypeScore - tempMinTypeScoreShao[i])<200))
		{
			...
			else
			{
				...
				ShiSanZhangOutCardCeLue(cbHandCardData, cbHandCardCount, Array, CardTypeResult, bChongSan);
...
void CGameLogicNew::ShiSanZhangOutCardCeLue(const BYTE cbHandCardData[], BYTE cbHandCardCount, BYTE  Array[DOU_NUM][DOU_HAND_COUNT], tagOutCardTypeResultNew * CardTypeResult,bool bChongSan)
{
	BYTE cbMaxCard = 0;
	...
	if (bChongSan)
	{
		if (type0 == CT_FIVE_THREE_DEOUBLE &&type2==CT_THREE )
		{
			SortOutCardList(Array[0], DOU_HAND_COUNT);

			if (GetCardLogicValue(Array[0][2])>GetCardLogicValue(Array[2][2]) )
			{
				SwitchArray(&Array[0][0], &Array[2][0], 3);
			}
		}
		 if ( type1 == CT_FIVE_THREE_DEOUBLE && type2 == CT_THREE)
		{
			 SortOutCardList(Array[1], DOU_HAND_COUNT);
			 if (GetCardLogicValue(Array[1][2]) > GetCardLogicValue(Array[2][2]))
			 {
				 SwitchArray(&Array[1][0], &Array[2][0], 3);
			 }
		}
	}
	...
	if (type0 == CT_FIVE_THREE_DEOUBLE&& type1 == CT_FIVE_THREE_DEOUBLE&& type2 == CT_SINGLE)
	{
		...
		/*if ((GetCardLogicValue(Array[0][3]) >= 13 || GetCardLogicValue(Array[1][3]) >= 13) && bExist == false)
		{
