...
CGameLogic::CGameLogic()
{
	...
	m_cbMagicIndex = SwitchToCardIndex(0x35);
	SetMagicIndex(m_cbMagicIndex);
}
...
WORD CGameLogic::GetChiHuActionRank(const CChiHuRight & ChiHuRight)
{
	...
	if( !(ChiHuRight&CHR_QING_LONG_QI_DUI).IsEmpty() )
		...
		wFanShu = 4;
	else if( !(ChiHuRight&CHR_DA_DAN_DIAO).IsEmpty() )
		wFanShu = 4;
...
BYTE CGameLogic::AnalyseChiHuCard(const BYTE cbCardIndex[MAX_INDEX], const tagWeaveItem WeaveItem[], BYTE cbWeaveCount, BYTE cbCurrentCard, CChiHuRight &ChiHuRight)
{
	...
	//必须缺一门
	//if( IsHuaZhu(cbCardIndexTemp,WeaveItem,cbWeaveCount,0) ) return WIK_NULL;
	...
	if (AnalyseItemArray.size()>0)
	{
		...
		//牌型分析
		for (INT_PTR i=0;i<AnalyseItemArray.size();i++)
		{
			//变量定义
...
//分析扑克
bool CGameLogic::AnalyseCard(const BYTE cbCardIndex[MAX_INDEX], const tagWeaveItem WeaveItem[], BYTE cbWeaveCount, vector<tagAnalyseItem>  & AnalyseItemArray)
{
	//计算数目
...
bool CGameLogic::IsDaDanDiao(const BYTE cbCardIndex[])
{
	//吃胡时只能有一对牌
	int count = 0;
	for (BYTE i = 0; i < MAX_INDEX; i++)
	...
	else
	{
		...
		for (BYTE j = 0; j < MAX_INDEX; j++)
		{
			...
			if (WIK_CHI_HU == AnalyseChiHuCard(cbCardIndexTemp, WeaveItem, cbWeaveCount, cbCurrentCard, chr))
			{
				...
				cbHuFan[0][cbCount-1] = GetChiHuActionRank(chr);
			}
		}
...
BYTE CGameLogic::SelectCallCard(BYTE cbCardIndex[MAX_INDEX])const
{
   ...
   for (int i=0;i<MAX_INDEX;i++)
   {
     if (i<9)
     {
		 count1+=cbCardIndex[i];
     }
	 else if (i<18)
	 {
		 count2+=cbCardIndex[i];
	 }
	 else
		  count3+=cbCardIndex[i];
   }
   BYTE count=min(min(count1,count2),count3);
   if (count==count1)
   {
      return M_WAN;
   }
