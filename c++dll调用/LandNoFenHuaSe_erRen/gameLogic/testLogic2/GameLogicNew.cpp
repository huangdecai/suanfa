...
bool CGameLogicNew::OutCardShengYuFenCheck(BYTE cbHandCardCount, const BYTE * cbHandCardData, const BYTE cbTurnCardData[], BYTE cbTurnCardCount, WORD wOutCardUser, float MinTypeScoreVec, vector<tagOutCardResultNew> &vecMinTypeCardResult, tagOutCardResultNew & OutCardResult)
{
	...
	if (bExistBiYing&&vecIndexUnIncludeBomb.size()>0)
	{
		resultIndex = vecIndexUnIncludeBomb[0];
	}
...
bool CGameLogicNew::FindMaxTypeTakeOneType(const BYTE cbHandCardData[], BYTE cbHandCardCount, BYTE cbTurnCardType, const BYTE cbTurnCardData[], BYTE cbTurnCardCount, vector<tagOutCardResultNew> & vecMinTypeCardResult, tagOutCardResultNew &OutCardResult)
{
	if (vecMinTypeCardResult.size() != 2)
	{
		...
		{
			int type = vecMinTypeCardResult[0].cbCardType;
			if (type==CT_FOUR_TAKE_ONE)
			{
				BYTE cbRemainCard[MAX_COUNT] = {};
				BYTE cbRemainCardCount =  vecMinTypeCardResult[0].cbCardCount-4;
				CopyMemory(cbRemainCard, cbHandCardData, vecMinTypeCardResult[0].cbCardCount);
				RemoveCard(vecMinTypeCardResult[0].cbResultCard, 4, cbRemainCard, vecMinTypeCardResult[0].cbCardCount);
				if (cbRemainCard[0]==cbRemainCard[1])
				{
					...
					bool bExistMax = SearchOtherHandCardThan(&cbRemainCard[0], 1, true);
					if (bExistMax == false)
					{
						ZeroMemory(&OutCardResult, sizeof(OutCardResult));
						OutCardResult.cbCardCount = 1;
						...
						bool bExistMax = SearchOtherHandCardThan(&cbRemainCard[2], 1, true);
						if (bExistMax == false)
						{
							ZeroMemory(&OutCardResult, sizeof(OutCardResult));
							OutCardResult.cbCardCount = 1;
...
bool CGameLogicNew::ThreeTakeTwoTakeMinCard(const BYTE cbCardData[], BYTE cbHandCardCount, vector<tagOutCardResultNew> &vecMinTypeCardResult, tagOutCardResultNew & OutCardResult, tagOutCardTypeResultNew * CardTypeResult)
{
	...
	if (outCardType == CT_THREE_TAKE_TWO)
	{
		...
		for (int i = 0; i < CardTypeResult[CT_DOUBLE].cbCardTypeCount; i++)
		{
			if (GetACardCount(cbThreeCard, 3 * tempCount, CardTypeResult[CT_DOUBLE].cbCardData[i][0])==0)
			{
				cbRemainCardData[cbRemainCardCount++] = CardTypeResult[CT_DOUBLE].cbCardData[i][0];
				cbRemainCardData[cbRemainCardCount++] = CardTypeResult[CT_DOUBLE].cbCardData[i][1];
			}
		}
		SortCardList(cbRemainCardData, cbRemainCardCount, ST_ORDER);
		int minTypeCount = 0;
