...
BYTE CGameLogicNew::SearchOutCard(const BYTE cbHandCardData[], BYTE cbHandCardCount, const BYTE cbTurnCardData[], BYTE cbTurnCardCount, tagSearchCardResult *pSearchCardResult, bool bNoSearchBomb)
{
	...
	{
		BYTE cbTakeCount = cbTurnOutType == CT_FOUR_TAKE_ONE ? 2 : 4;
...
float CGameLogicNew::GetHandScore(vector<tagOutCardResultNew> &CardTypeResult, int minTypeCount,bool bJiaoFen)
{
	...
	for (int i = 0; i < CardTypeResult.size(); i++)
	{
		...
		else if (CardTypeResult[i].cbCardType == CT_MISSILE_CARD)
		{
			score += 50;
			bHaveWang = 2;
		}
	...
	if (bJiaoFen&&bHave2&&bHaveBoombCount == 0 && bHaveWang == 0)
	{
		score -= 30;
	}
	...
	if (cbTmpResultCount > 0)
	{
		...
		for (BYTE i = 0; i < cbTmpResultCount; i++)
		{
			...
			if (cbTakeCardCount >= 2)
			{
				//搜索同张
				tagSearchCardResult SameCardResult = {};
				BYTE cbSameCardResultCount = SearchSameCard(cbRemainCard, cbRemainCardCount, 0, 2, &SameCardResult);
				ZeroMemory(cbRemainCard, sizeof(cbRemainCard));
				cbRemainCardCount = 0;
				for (BYTE i = 0; i < SameCardResult.cbSearchCount; i++)
				{
					CopyMemory(cbRemainCard+cbRemainCardCount, SameCardResult.cbResultCard[i], SameCardResult.cbCardCount[i]);
					cbRemainCardCount += 2;
				}
			}
			if (cbTakeCardCount>=2)
...
int CGameLogicNew::YouXianDaNengShouHuiCard(const BYTE cbHandCardData[], BYTE cbHandCardCount, vector<tagOutCardResultNew>& vecMinTypeCardResult, tagOutCardResultNew & OutCardResult, bool &bZhiJieChu)
{
	...
	if (bZhiJieChu)
	{
		...
		else if ((juDuiMaxIndex[CT_MISSILE_CARD] + juDuiMaxIndex[CT_FOUR_TAKE_TWO] + juDuiMaxIndex[CT_FOUR_TAKE_ONE] + juDuiMaxIndex[CT_BOMB_CARD]) == (vecMinTypeCardResult.size() - 1) && m_cbUserCardCount[1] != vecMinTypeCardResult[vecMinTypeCardResult.size() - 1].cbCardCount)
		{
			for (int i = 0; i < vecMinTypeCardResult.size(); i++)
			{
				int type = vecMinTypeCardResult[i].cbCardType;
				...
				{
					int szie = vecMinTypeCardResult[i].cbCardCount;
					if (vecMinTypeCardResult[i].cbResultCard[szie - 1] == vecMinTypeCardResult[i].cbResultCard[szie-2])
					{
						BYTE tmpTurnCard[MAX_COUNT] = { 0 };
						int tmpTurnCardCount = 2;
						CopyMemory(tmpTurnCard, vecMinTypeCardResult[i].cbResultCard + 4, tmpTurnCardCount);
						bool bExistMax = SearchOtherHandCardThan(tmpTurnCard, tmpTurnCardCount, true);
						if (bExistMax == false)
						{
							resultIndex = i;
							OutCardResult.cbCardCount = tmpTurnCardCount;
							OutCardResult.cbCardType = CT_DOUBLE;
							CopyMemory(OutCardResult.cbResultCard, tmpTurnCard, OutCardResult.cbCardCount);
							return resultIndex;
						}
					}
				}
				else if (type == CT_THREE_TAKE_ONE&& vecMinTypeCardResult[i].cbCardCount==4)
				{
						BYTE tmpTurnCard[MAX_COUNT] = { 0 };
						int tmpTurnCardCount = 1;
						CopyMemory(tmpTurnCard, vecMinTypeCardResult[i].cbResultCard + 3, tmpTurnCardCount);
						bool bExistMax = SearchOtherHandCardThan(tmpTurnCard, tmpTurnCardCount, true);
						if (bExistMax == false)
						{
							resultIndex = i;
			...
			resultIndex = vecMinTypeCardResult.size() - 1;
		}
...
bool CGameLogicNew::OutCardShengYuFenCheck(BYTE cbHandCardCount, const BYTE * cbHandCardData, const BYTE cbTurnCardData[], BYTE cbTurnCardCount, WORD wOutCardUser, float MinTypeScoreVec, vector<tagOutCardResultNew> &vecMinTypeCardResult, tagOutCardResultNew & OutCardResult)
{
	...
	for (int i = 0; i < SearchCardResult.cbSearchCount; i++)
	{
		...
		if (cbReserveCardCount > 0)
		{
			...
			else if ((bExistMax != true) && (tempType <CT_FOUR_TAKE_ONE) )
			{
				//大一手，炸弹和大小王带两手就获胜的牌
				int tmpMaxCount = 0;
				if (TempMinTypeCardResult[i][0].cbCardType >= CT_BOMB_CARD  )
				{
					for (int j = 1; j < TempMinTypeCardResult[i].size(); j++)
					{
						if ((TempMinTypeCardResult[i][0].cbCardCount + tmpMaxCount + TempMinTypeCardResult[i][j].cbCardCount)>=m_cbUserCardCount[0])
						{
							BiShengResult.push_back(i);
							bExistBiYing = true;
							resultIndex = i;
						...
						else if (TempMinTypeCardResult[i][j].cbCardCount>tmpMaxCount)
						{
							tmpMaxCount = TempMinTypeCardResult[i][j].cbCardCount;
						}
			...
			else if (bExistBiYing==false&&tempType == CT_DOUBLE&&GetCardLogicValue(SearchCardResult.cbResultCard[i][0]) >= 13 && WuDiCheck(cbReserveCardData, cbReserveCardCount, NULL, 0, OutCardResult))
			{
				MinTypeScore = tempMinTypeScore;
				minTypeCount = tempMinTypeCount;
				resultIndex = i;
	...
	if ((resultIndex != -1))
	{
		...
		if (resultIndex != -1 && bExistBiYing&&vecIndexUnIncludeBomb.size() > 0)
		{
			...
			if ((reIndex == XIANG_DUI_DA || reIndex == JUE_DUI_DA) && m_cbUserCardCount[1] != SearchCardResult.cbCardCount[resultIndex] && bHaveBombTypeCount>0)
			{
				int a = 0;
			}
			else{
	...
	if ((resultIndex != -1) && bExistBiYing==false)
	{
		...
		if (tempCardScore>20)
		{
			...
			{
				if (tempCardType == CT_MISSILE_CARD&&m_cbUserCardCount[1] == 1 &&  cbTurnCardCount==1)
				{
					//拆牌顶个王
					ZeroMemory(&OutCardResult, sizeof(OutCardResult));
					OutCardResult.cbCardCount = 1;
			...
			{
				if (bHaveBombTypeCount<=1&&(m_cbUserCardCount[1] >= 10) || (m_cbUserCardCount[1] <= 2))
				{
					resultIndex = -1;
				}
			}
...
bool CGameLogicNew::WuDiCheck(const BYTE * cbHandCardData, BYTE cbHandCardCount, const BYTE * cbTurnCardData, BYTE cbTurnCardCount, tagOutCardResultNew &OutCardResult, tagOutCardTypeResultNew *CardTypeResult)
{
	...
	for (int i = 0; i < SearchCardResult.cbSearchCount; i++)
	{
		int type = GetCardType(SearchCardResult.cbResultCard[i], SearchCardResult.cbCardCount[i]);
		bool	bExistMax = SearchOtherHandCardThan(SearchCardResult.cbResultCard[i], SearchCardResult.cbCardCount[i], false);
		if (bExistMax == false && type>=CT_BOMB_CARD)
		{
			bombTypeIndex = i;
			bombCard = SearchCardResult.cbResultCard[i][0];
			break;
		}
	}
	//如果是出一手加一个炸弹刚好出完，先出没有炸弹的一手
	for (int i = 0; i < SearchCardResult.cbSearchCount; i++)
	{
		if (bombTypeIndex != -1 && i!=bombTypeIndex)
		{
			if (GetACardCount(SearchCardResult.cbResultCard[i],SearchCardResult.cbCardCount[i], bombCard)>= 1)
			{
				continue;
			}
			if ((SearchCardResult.cbCardCount[i] + SearchCardResult.cbCardCount[bombTypeIndex]) == m_cbUserCardCount[0] && SearchCardResult.cbCardCount[i] != m_cbUserCardCount[1])
			{
				int index = i;
				ZeroMemory(&OutCardResult, sizeof(OutCardResult));
	...
	if (vecMinTypeCardResult.size() != 2)
	{
		...
		for (int i = 0; i < vecMinTypeCardResult.size(); i++)
		{
			if (vecMinTypeCardResult[i].cbCardType == CT_BOMB_CARD || vecMinTypeCardResult[i].cbCardType == CT_MISSILE_CARD || cbTurnCardType == 0)
			{
				...
				if (bExistMax == false||vecMinTypeCardResult[i].cbCardCount >= m_cbUserCardCount[0])
				{
					ZeroMemory(&OutCardResult, sizeof(OutCardResult));
					if (FourTakeFenChai(vecMinTypeCardResult[i].cbResultCard, vecMinTypeCardResult[i].cbCardCount, vecMinTypeCardResult, OutCardResult))
					{
						...
						if (vecMinTypeCardResult.size()==2 && vecMinTypeCardResult[1].cbCardCount >= 8)
						{
							OutCardResult.cbCardCount = vecMinTypeCardResult[1].cbCardCount;
							CopyMemory(OutCardResult.cbResultCard, vecMinTypeCardResult[1].cbResultCard, OutCardResult.cbCardCount);
						}
...
bool CGameLogicNew::FindTrunMaxTypeTakeOneType(const BYTE cbHandCardData[], BYTE cbHandCardCount, BYTE cbTurnCardType, const BYTE cbTurnCardData[], BYTE cbTurnCardCount, vector<tagOutCardResultNew> & vecMinTypeCardResult, tagOutCardResultNew &OutCardResult)
{
	if (vecMinTypeCardResult.size() != 2)
	{
		return false;
	...
	ZeroMemory(&OutCardResult, sizeof(OutCardResult));
	if ( vecMinTypeCardResult[0].cbCardType>= CT_BOMB_CARD &&vecMinTypeCardResult[1].cbCardType == CT_FOUR_TAKE_ONE )
		{
			if (cbTurnCardCount == 1)
			{
				...
				for (int i = 0; i < singleData.size(); i++)
				{
					if (GetCardLogicValue(singleData[i]) <= GetCardLogicValue(cbTurnCardData[0]))
					{
						continue;
					}
					BYTE tmpTurnCard[MAX_COUNT] = { 0 };
	...
	else if (vecMinTypeCardResult[0].cbCardType >= CT_BOMB_CARD &&vecMinTypeCardResult[1].cbCardType == CT_FOUR_TAKE_TWO)
		{
...
VOID CGameLogicNew::ErBeiDongOutCard(const BYTE cbHandCardData[], BYTE cbHandCardCount, WORD wOutCardUser, const BYTE cbTurnCardData[], BYTE cbTurnCardCount, tagOutCardResultNew & OutCardResult)
{
	...
	if (FindTrunMaxTypeTakeOneType(cbHandCardData, cbHandCardCount, cbOutCardType, cbTurnCardData, cbTurnCardCount, vecMinTypeCardResult, OutCardResult))
	{
		return;
	}
	if (minTypeCount == 1 && CompareCard(cbTurnCardData, cbHandCardData, cbTurnCardCount, cbHandCardCount))
...
void CGameLogicNew::BaoDanJiaoYan(tagOutCardResultNew &OutCardResult, WORD wUndersideUser, const BYTE * cbHandCardData)
{
	if (OutCardResult.cbCardCount == 1 && (m_cbUserCardCount[wUndersideUser] == 1) )
	{
		int reIndex = IsJueDuiDaPai(OutCardResult.cbResultCard, OutCardResult.cbCardCount, CT_SINGLE);
		if ((reIndex == XIANG_DUI_DA || reIndex == JUE_DUI_DA))
		{
			int a = 0;
		}
		else{
...
bool CGameLogicNew::ThreeTakeTwoTakeMinCard(const BYTE cbCardData[], BYTE cbHandCardCount, vector<tagOutCardResultNew> &vecMinTypeCardResult, tagOutCardResultNew & OutCardResult, tagOutCardTypeResultNew * CardTypeResult)
{
	...
	int outCardType = GetCardType(OutCardResult.cbResultCard, OutCardResult.cbCardCount);
	bool bEnalbe = true;
	if (OutCardResult.cbCardCount >= 8 && vecMinTypeCardResult.size() <= 2)
	{
		bEnalbe = false;
	}
	
	if ((outCardType == CT_FOUR_TAKE_ONE || outCardType == CT_THREE_TAKE_TWO || outCardType == CT_THREE_TAKE_ONE) && bEnalbe)
	{
	...
	if (m_cbUserCardCount[0]<=4)
	{
		...
		if ((singleData.size() >= 1||doubleData.size()>=1) && outtype == CT_FOUR_TAKE_ONE)
		{
			...
			for (int i = 0; i < singleData.size(); i++)
			{
				...
				else if (tmpTurnCard[0] == 14 && vecMinTypeCardResult.size() == 2 && singleData.size()==3)
				{
					int tempIndex = singleData.size() - 1;
					OutCardResult.cbCardCount = 1;
					OutCardResult.cbCardType = CT_SINGLE;
			...
			else if (m_cbUserCardCount[1]>1 )
			{
				...
				else if (cbHandCardCount>m_cbUserCardCount[0])
				{
					OutCardResult.cbCardCount = 1;
					CopyMemory(OutCardResult.cbResultCard, &singleData[singleData.size() - 1], OutCardResult.cbCardCount);
					return true;
		...
		if (m_cbUserCardCount[1] != 2 )
		{
			...
			else if (vecMinTypeCardResult.size() == 2)
			{
				if (typeCount[CT_DOUBLE]==1)
				{
					...
					OutCardResult.cbCardCount = 2;
					OutCardResult.cbCardType = CT_DOUBLE;
					CopyMemory(OutCardResult.cbResultCard, &doubleData[tempIndex], 2);
					return true;
				}
			}
			else if ( m_cbUserCardCount[0]<=6)
			{
				int tempIndex = doubleData.size() - 2;
				OutCardResult.cbCardCount = 2;
				OutCardResult.cbCardType = CT_DOUBLE;
...
bool CGameLogicNew::ThreeTakeMinCard(const BYTE cbCardData[], BYTE cbHandCardCount, vector<tagOutCardResultNew> &vecMinTypeCardResult, tagOutCardResultNew & OutCardResult, tagOutCardTypeResultNew * CardTypeResult)
{
	...
	if (outCardType == CT_THREE)
	{
		...
		OutCardResult.cbCardCount += tempIndex;
		CopyMemory(OutCardResult.cbResultCard, ResultThreeTakeCard[tempIndex], OutCardResult.cbCardCount);
		return true;
	}
	return false;
}
