...
{
	0x01,     0x03,0x04,0x05,0x06,0x07,0x08,0x09,0x0A,0x0B,0x0C,	//方块 A - K
			...
			while (i <= 12)
			{
				...
				if (tmpSearchCardResult.cbSearchCount > 0)
				{
					...
					for (int j = 0; j < tmpSearchCardResult.cbSearchCount; j++)
					{
						...
						BYTE  cbTmpSingleLineCardData[MAX_COUNT] = { 0 };
						CopyMemory(cbTmpSingleLineCardData, CardTypeResult[CT_SINGLE_LINE].cbCardData[Index], tmpSearchCardResult.cbCardCount[j]);
						bool bEnable = false;
						for (int k = 0; k < tmpSearchCardResult.cbCardCount[j]; k++)
						{
							for (int m = 0; m < cbHandCardCount;m++)
							{
								if ((cbTmpSingleLineCardData[k] != cbHandCardData[m])
									&&( GetCardLogicValue(cbTmpSingleLineCardData[k]) == GetCardLogicValue(cbHandCardData[m])))
								{
									cbTmpSingleLineCardData[k] = cbHandCardData[m];
									bEnable = true;
								}
						...
						if (bEnable)
						{
							int Index = CardTypeResult[CT_SINGLE_LINE].cbCardTypeCount;
							CardTypeResult[CT_SINGLE_LINE].cbCardType = CT_SINGLE_LINE;
							CopyMemory(CardTypeResult[CT_SINGLE_LINE].cbCardData[Index], cbTmpSingleLineCardData, tmpSearchCardResult.cbCardCount[j]);
							CardTypeResult[CT_SINGLE_LINE].cbEachHandCardCount[Index] = tmpSearchCardResult.cbCardCount[j];
							CardTypeResult[CT_SINGLE_LINE].cbCardTypeCount++;
						}
					}
...
bool CGameLogicNew::SearchOutCardErRen(BYTE cbHandCardData[], BYTE cbHandCardCount, BYTE cbTurnCardData[], BYTE cbTurnCardCount, BYTE	DiscardCard[], BYTE cbDiscardCardCount, BYTE cbRangCardCount, BYTE cbOthreRangCardCount, tagOutCardResultNew & OutCardResult)
{
	...
	m_cbUserCardCount[1] = (FULL_COUNT - cbHandCardCount - cbDiscardCardCount - NORMAL_COUNT);
...
VOID CGameLogicNew::ErZhuDongOutCard(const BYTE cbHandCardData[], BYTE cbHandCardCount, tagOutCardResultNew & OutCardResult)
{
	...
	if (FindMaxTypeTakeOneType(cbHandCardData, cbHandCardCount, 0,0, 0, vecMinTypeCardResult, OutCardResult))
	{
		...
		if (type == CT_THREE || type == CT_THREE_TAKE_ONE)
		{
