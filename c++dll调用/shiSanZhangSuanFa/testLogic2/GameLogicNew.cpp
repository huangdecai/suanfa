...
void CGameLogicNew::GetDoubleResult(BYTE * cbHandCardData, BYTE const cbHandCardCount, tagOutCardTypeResultNew * CardTypeResult)
{
	...
	for (BYTE i = 0; i < SameCardResult.cbSearchCount; i++)
	{
		if (IsReResult(CardTypeResult, CT_ONE_DOUBLE, SameCardResult.cbResultCard[i], SameCardResult.cbCardCount[i]) == false)
		{
			BYTE Index = CardTypeResult[CT_ONE_DOUBLE].cbCardTypeCount;
			CardTypeResult[CT_ONE_DOUBLE].cbCardType = CT_ONE_DOUBLE;
			CopyMemory(CardTypeResult[CT_ONE_DOUBLE].cbCardData[Index], SameCardResult.cbResultCard[i], SameCardResult.cbCardCount[i]);
			CardTypeResult[CT_ONE_DOUBLE].cbEachHandCardCount[Index] = SameCardResult.cbCardCount[i];
			CardTypeResult[CT_ONE_DOUBLE].cbCardTypeCount++;
		}
	}
}
...
void CGameLogicNew::GetBombCardResult(BYTE * cbHandCardData, BYTE const cbHandCardCount, tagOutCardTypeResultNew * CardTypeResult)
{
	...
	for (BYTE i = 0; i < cbFourCardCount; i += 4)
	{
		...
		RemoveCard(&cbFourCardData[i], 4, cbRemainCarData, cbHandCardCount);
		...
		for (int j = 0; j < cbRemainCardCount;j++)
		{
			...
			CardTypeResult[CT_FIVE_FOUR_ONE].cbCardType = CT_FIVE_FOUR_ONE;
			CardTypeResult[CT_FIVE_FOUR_ONE].cbCardData[Index][0] = cbFourCardData[i];
			CardTypeResult[CT_FIVE_FOUR_ONE].cbCardData[Index][1] = cbFourCardData[i + 1];
			CardTypeResult[CT_FIVE_FOUR_ONE].cbCardData[Index][2] = cbFourCardData[i + 2];
			CardTypeResult[CT_FIVE_FOUR_ONE].cbCardData[Index][3] = cbFourCardData[i + 3];
...
void CGameLogicNew::GetThreeTakeTwoCardResult(BYTE * cbHandCardData, BYTE const cbHandCardCount, tagOutCardTypeResultNew * CardTypeResult)
{
	...
	for (BYTE i = 0; i < cbHandThreeCount; i += 3)
	{
		...
		/*	for (BYTE j = 0; j < cbAllDoubleCardCount; j += 2)
			...
			CardTypeResult[CT_FIVE_THREE_DEOUBLE].cbCardData[Index][0] = cbHandThreeCard[i];
			CardTypeResult[CT_FIVE_THREE_DEOUBLE].cbCardData[Index][1] = cbHandThreeCard[i + 1];
			CardTypeResult[CT_FIVE_THREE_DEOUBLE].cbCardData[Index][2] = cbHandThreeCard[i + 2];
			CardTypeResult[CT_FIVE_THREE_DEOUBLE].cbCardData[Index][3] = cbAllDoubleCardData[j];
			CardTypeResult[CT_FIVE_THREE_DEOUBLE].cbCardData[Index][4] = cbAllDoubleCardData[j + 1];
		...
		for (BYTE j = 0; j < CardTypeResult[CT_ONE_DOUBLE].cbCardTypeCount; j++)
			{
				if (GetCardLogicValue(CardTypeResult[CT_ONE_DOUBLE].cbCardData[j][0]) !=
					GetCardLogicValue(cbHandThreeCard[i]))
				{
					BYTE Index = CardTypeResult[CT_FIVE_THREE_DEOUBLE].cbCardTypeCount;
					CardTypeResult[CT_FIVE_THREE_DEOUBLE].cbCardType = CT_FIVE_THREE_DEOUBLE;
					CardTypeResult[CT_FIVE_THREE_DEOUBLE].cbCardData[Index][0] = cbHandThreeCard[i];
		...
		if (G_THREE_TAKE_TWO_DAN)
		{
			...
			//	CardTypeResult[CT_THREE_TAKE_ONE].cbCardData[Index][3] = cbComResCard[m][0];
			//	CardTypeResult[CT_THREE_TAKE_ONE].cbCardData[Index][4] = cbComResCard[m][1];
			//	CardTypeResult[CT_THREE_TAKE_ONE].cbEachHandCardCount[Index] = 5;
	...
	while (cbLeftThreeCardCount >= 6 && bFindThreeLine)
	{
		...
		if (cbThreeLineCardCount > 3)
		{
			...
			for (BYTE start = 0; start < cbThreeLineCardCount - 3; start += 3)
			{
				...
				for (BYTE i = 0; i < cbComResLen; ++i)
				{
					Index = CardTypeResult[CT_FIVE_THREE_DEOUBLE].cbCardTypeCount;
					CardTypeResult[CT_FIVE_THREE_DEOUBLE].cbCardType = CT_FIVE_THREE_DEOUBLE;
					////保存三条
					CopyMemory(CardTypeResult[CT_FIVE_THREE_DEOUBLE].cbCardData[Index], cbThreeLineCard + start, cbThisTreeLineCardCount);
					CopyMemory(&CardTypeResult[CT_FIVE_THREE_DEOUBLE].cbCardData[Index][cbThisTreeLineCardCount], cbComResCard[i], cbSingleCardCount);
					CardTypeResult[CT_FIVE_THREE_DEOUBLE].cbEachHandCardCount[Index] = cbThisTreeLineCardCount + 2 * cbDoubleCardCount;
					CardTypeResult[CT_FIVE_THREE_DEOUBLE].cbCardTypeCount++;
...
VOID CGameLogicNew::SortOutCardList(BYTE cbCardData[], BYTE cbCardCount)
{
	...
	if (cbCardType == CT_TWO_DOUBLE)
	{
		...
		cbCardCount = AnalyseResult.cbBlockCount[1] * 2;
		CopyMemory(cbCardData, AnalyseResult.cbCardData[1], sizeof(BYTE)*cbCardCount);
		for (int i = CountArray(AnalyseResult.cbBlockCount) - 1; i >= 0; i--)
		{
			if (i == 1) continue;
	...
	for (BYTE i=0;i<cbHandCardCount;i++)
	{
		...
		if(4==cbSameCount)
		{
			...
			if (CardTypeResult != NULL)
			{
				...
				CopyMemory(CardTypeResult[CT_FIVE_MIXED_FLUSH_NO_A].cbCardData[Index], &cbTmpCardData[i], sizeof(BYTE)*cbSameCount);
				CardTypeResult[CT_FIVE_MIXED_FLUSH_NO_A].cbEachHandCardCount[Index] = cbSameCount;
...
BYTE CGameLogicNew::SearchAllLineCardType(const BYTE cbHandCardData[], BYTE cbHandCardCount, BYTE cbLineCount, tagSearchCardResult *pSearchCardResult)
{
	...
	for (BYTE cbValueIndex = 0; cbValueIndex < 14; cbValueIndex++)
	{
		...
		if (cbTmpLinkCount >= cbLineCount)
		{
			...
			GetNextLineCard(cbValueIndex + 1 - cbTmpLinkCount, cbValueIndex, cbLineCount, lineArray, Distributing, pSearchCardResult);
...
void CGameLogicNew::GetNextLineCard(BYTE cbIndexSatart, BYTE cbIndexEnd, BYTE cbLineCount, vector<BYTE> & lineArray, tagDistributingNew& Distributing, tagSearchCardResult *pSearchCardResult)
{
	//复制扑克
	BYTE cbCount = 0;
	...
	for (BYTE cbIndex = cbIndexSatart; cbIndex <= cbIndexEnd; cbIndex++)
	{
		...
		for (BYTE cbColorIndex = 0; cbColorIndex < 4; cbColorIndex++)
		{
			for (BYTE cbColorCount = 0; cbColorCount < Distributing.cbDistributing[cbIndex][3 - cbColorIndex]; cbColorCount++)
			{
				...
				{
					GetNextLineCard(cbIndex + 1, cbIndexEnd, cbLineCount, lineArray, Distributing, pSearchCardResult);
				}
...
bool CGameLogicNew::IsReResult(tagOutCardTypeResultNew * CardTypeResult, int type, BYTE cbCardData[], BYTE cbCardCount)
{
	...
	for (int k = 0; k < cbCardCount; k++)
	{
		...
		for (BYTE i = 0; i < CardTypeResult[type].cbCardTypeCount; i++)
		{
			...
			{
				if (cbCardData[k] == CardTypeResult[type].cbCardData[i][j])
				{
					bSame = true;
					break;
				}
