...
//模拟操作
void CAndroidAI::SetAction(BYTE byActionMask, BYTE byActionCard, int GangKind )
{
	//验证
...
bool CAndroidAI::SearchOutCard(tagOutCardResult &OutCardResult, WORD wMeChairId, WORD  wCurrentUser, BYTE cbCallCard, BYTE  cbActionMask, BYTE	cbActionCard,  BYTE cbCardIndex[], tagWeaveItem WeaveItemArray[GAME_PLAYER][MAX_WEAVE], BYTE cbWeaveCount[], BYTE cbDiscardCard[], BYTE cbDiscardCount)
{
	//初始化
	...
	for (BYTE i = 0; i < MAX_INDEX; i++)
	{
		if (i == m_GameLogic.SwitchToCardIndex(0x35))
		{
			for (BYTE j = 0; j < cbCardIndex[i]; j++)
			{
	...
	else if (wCurrentUser == wMeChairId)
	{
		...
		if (cbRes == WIK_LISTEN)
		{
			...
			for (int i = 0; i < MAX_COUNT; i++)
			{
				...
				if (FanCount>maxFanCount&&cbHuCardRemainingCount>=2)
				{
					maxFanCount = FanCount;
					huPaiIndex = i;
					nOrgScore = -1;
	...
	//计算各种操作得分
	BYTE cbOperateCode[] = { WIK_GANG, WIK_PENG, WIK_NULL };
	int nOperateScore[] = { 0, 0, 0 };
	//计算原始分
	CAndroidAI AndroidAi;
	AndroidAi.SetEnjoinOutCard(cbEnjoinOutCardIndex, cbEnjoinOutCardCount);
	AndroidAi.SetCardData(byCard, byCardCount, WeaveItemArray[wMeChairId], cbWeaveCount[wMeChairId]);
	...
	if (cbActionMask & WIK_GANG)
	{
		...
		else
		{
			...
			int score = ActionAfterScore(wMeChairId, cbCardIndex, cbShengYuIndex, WeaveItemArray[wMeChairId], cbWeaveCount[wMeChairId], cbDiscardCard, cbDiscardCount, cbActionCard, WIK_GANG);
	...
	if (cbActionMask & WIK_PENG&&wMeChairId != wCurrentUser)
	{
		...
		int score = ActionAfterScore(wMeChairId, cbCardIndex, cbShengYuIndex, WeaveItemArray[wMeChairId], cbWeaveCount[wMeChairId], cbDiscardCard, cbDiscardCount, cbActionCard, WIK_PENG);
	...
	if ((cbOperateCode[cbIndex] & cbActionMask) &&
...
int  CAndroidAI::ActionAfterScore(WORD wMeChairId, BYTE * cbCardIndex, BYTE * cbShengYuIndex, tagWeaveItem * WeaveItemArray, BYTE  cbWeaveCount, BYTE * cbDiscardCard, BYTE cbDiscardCount, BYTE cbActionCard, int  actionType)
{
	cbCardIndex[m_GameLogic.SwitchToCardIndex(cbActionCard)]--;
	int totalNum = SearchTingTotalCount(wMeChairId, cbCardIndex, cbShengYuIndex, WeaveItemArray, cbWeaveCount, cbDiscardCard, cbDiscardCount);
	cbCardIndex[m_GameLogic.SwitchToCardIndex(cbActionCard)]++;
	int score = 0;
	...
	tmpWeaveCount++;
	int totalNum2 = SearchTingTotalCount(wMeChairId, cbTmpCardIndex, cbShengYuIndex, tmpWeaveItemArray, tmpWeaveCount, cbDiscardCard, cbDiscardCount);
	if (totalNum < totalNum2)
	{
		score = 500;
	}
	else if ((actionType == WIK_GANG || actionType == WIK_PENG) && totalNum == totalNum2)
	{
		score = 500;
	}
	else{
...
int CAndroidAI::SearchTingTotalCount(WORD wMeChairId, BYTE cbCardIndex[], BYTE cbShengYuIndex[MAX_INDEX], tagWeaveItem WeaveItemArray[MAX_WEAVE], BYTE cbWeaveCount, BYTE cbDiscardCard[], BYTE cbDiscardCount)
{
	int totalNum = 0;
