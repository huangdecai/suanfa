...
class GameHelper:
    def __init__(self):
        ...
        self.Pics = {}
        self.PicsCV = {}
        self.Handle = win32gui.FindWindow("ywRbO3hjls9nocQkAOVccjZLe1u2pv7", "新倩女幽魂Online(畅玩版) 版本[721611] 服务器[卧虎藏龙] 玩家ID[1402510777] 次世代")
    ...
    def ClickOnImage(self, templateName, region=None, confidence=0.9):
        ...
        if result is not None:
            self.LeftClick((result[0]+20, result[1]+20))
    ...
    def GetCardsState(self, image,handCount):
        ...
        for i in range(0, handCount):
            ...
            checkSelect = 0
            result = LocateOnImage(imgCv, self.PicsCV["card_overlap"], region=(temp_x, 385, sw, 50), confidence=0.85)
            if result is not None:
                checkSelect = 1
