...
class GameHelper:
    ...
    def GetCardsState(self, image,handCount):
        ...
        cardStartPos = pyautogui.locate(needleImage=self.Pics["card_edge"], haystackImage=image,
                                        region=(10, 413, 880, 110), confidence=0.80)
        ...
        for i in range(0, handCount):
            ...
            checkSelect = 0
            result = LocateOnImage(imgCv, self.PicsCV["card_overlap"], region=(temp_x, spaceY - 30, spaceX, 15),
                                   confidence=0.85)
            result2 = LocateOnImage(imgCv, self.PicsCV["card_overlap2"], region=(temp_x, spaceY - 10, spaceX, 40),
                                    confidence=0.85)
    ...
    def GetCards(self, image,handCount):
        ...
        for i in range(0, MAX_CARD_COUNT):
            ...
            if temp_x >= (SCREEN_WIDTH - 50):
                break
            print(i,temp_x,spaceX)
            checkSelect = 0
            result = LocateOnImage(imgCv, self.PicsCV["card_overlap"], region=(temp_x, spaceY-30, spaceX, 15), confidence=0.85)
            result2 = LocateOnImage(imgCv, self.PicsCV["card_overlap2"], region=(temp_x, spaceY-10, spaceX, 40), confidence=0.85)
            if (result ) and ((result2 is  None)):
                checkSelect = 1
            ...
            while ci < len(AllCardsNC):
                if "r" in AllCardsNC[ci] or "b" in AllCardsNC[ci]:
                    outerBreak = False
                    result = LocateOnImage(imgCv, self.PicsCV["m" + AllCardsNC[ci]], region=(sx + spaceX * i, spaceY- checkSelect * 25, sw, 68), confidence=0.95)
                    ...
                    outerBreak = False
                    for card_type in ["r", "b"]:
                        result = LocateOnImage(imgCv, self.PicsCV["m" + card_type + AllCardsNC[ci]], region=(sx + spaceX * i, spaceY - checkSelect * 25, sw, 68), confidence=0.89)
                        if result is not None:
                            cardPos = (sx + spaceX * i + sw // 2, spaceY - checkSelect * 25 + sh // 2)
                            cardSearchFrom = ci
                            currCard = AllCardsNC[ci]
        ...
        if (cardStartPos[0]>100 and cardStartPos[0]<500) and len(hand_cards)==0:
            print("cardStartPos-error:", cardStartPos)
            time.sleep(0.3)
            ...
            while ci < len(AllCardsNC):
                if "r" in AllCardsNC[ci] or "b" in AllCardsNC[ci]:
                    ...
                    result = LocateOnImage(imgCv, self.PicsCV["o" + AllCardsNC[ci]],
                                           region=(sx + spaceX * i, spaceY - checkSelect * 25, sw, 68), confidence=0.95)
    ...
    def SelectCards(self, cards,handCount, no_check=False):
        ...
        while 1 in clickMap:
            ...
            for i in range(0, len(clickMap)):
                tmpStr+=str(clickMap[i])+","
            print("SelectCards要点击:", tmpStr)
            for i in range(0, len(clickMap)):
                if clickMap[i] == 1:
